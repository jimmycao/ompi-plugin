...
orte_ess_base_module_t orte_ess_yarn_module = {
    rte_init,
    rte_finalize,
    rte_abort,
    NULL /* ft_event */
...
//	/* load pbc lib */
//	if (!lt_dlopenadvise("libpbc", advise)) {
//        free(search_path);
//		opal_output(0, "ess:yarn:init: lt_dlopenext for libpbc error: %s.\n", lt_dlerror());
//		return -1;
//	}

	if (!lt_dlopenadvise("libmca_hdclient", advise)) {
		opal_output(0, "ess:yarn:init: lt_dlopenext for libhdclient error: %s.\n", lt_dlerror());
		return -1;
	}
...
static int rte_init(void)
{
	...
	ret = ltdl_pbc();
	if (0 != ret) {
		opal_output(0, "ess:yarn:init: init pbc lib failed.\n");
		return ORTE_ERROR;
	}
